...
def noam_decay(step, warmup_steps, model_size):
    """Learning rate schedule described in paper 'Attention is all you need"""
    return (
        model_size ** (-0.5) *
        min(step ** (-0.5), step * warmup_steps ** (-1.5))
    )

def noamwd_decay(step, warmup_steps, model_size, rate = 0.5, decay_steps = 1000,
                 start_step = 500):
...
class Adam(Optimizer):
    """Implements BERT version of Adam algorithm with weight decay fix (and no ).
       Params:
       lr: learning rate
       warmup: protion of t_toal for the warmup. Default: -1
       t_total: total number of training steps for the learning rate
    ...
    def __init__(self, params, lr, warmup = -1, t_total = -1, schedule = 'warmup_linear',
        ...
        if not 0.0 <= b1 < 1.0:
            raise ValueError("Invalid b1 parameter: {} - shoule be in [0.0,1.0]".format(b1))
        if not 0.0 <= b2 < 1.0:
            raise ValueError("Invalid be parameter: {} - should be in [0.0,1.0]".format(b2))
        if not e  >= 0.0:
            raise ValueError("Invalid epsilon value: {} - should be >= 0.0".format(e))
        defaults = dict(lr=lr, schedule =schedule,warmup=warmup,
                        t_total=t_total,b1=b1,b2=b2,e=e,weight_decay_rate=weight_decay_rate,
                        max_grad_norm = max_grad_norm)
        super(Adam,self).__init__(params,defaults)

    def get_lr(self):
        lr = []
        for group in self.param_groups:  #params_groups contains those variables that need to be updated
            for p in group['params']:
                state = self.state[p] #self.state = {'params': , state}
                if len(state) == 0:
                    return [0]
                if group['t_total'] != -1:
                    schedule_fct = SCHEDULES[group['schedule']]
                    lr_scheduled = group['lr'] * schedule_fct(state['step']/group['total'],group['warmup'])
                else:
                    lr_scheduled = group['lr']
                lr.append(lr_scheduled)
        return lr  #return a list containing lr for each update step
    ...
    def step(self, closure = None):
        ...
        for group in self.param_groups:
            for p in group['params']:
                ...
                grad = p.grad.data
                if grad.is_sparse:
                    raise RuntimeError('Adam does not support sparse gradients, pleas consider SparseAdam instead')
                ...
                # bias_correction2 = 1 - beta ** state['step']
        return loss
